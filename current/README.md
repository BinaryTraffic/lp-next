...
| アプリ版 | **`1.4.0`**（`lp_reverse_cms/index.php` の `APP_VERSION`） |
| Git タグ | **`v1.2.0`**（`main` の本番想定）／過去: **`v1.1.0`**, **`v1.0.0`**。レガシー表記: `v1.1.11-stable` |

主な内容（v1.4.0 時点）: 資産解決の強化（`@import` / `url` / `srcset`）、**`data` / `output` 書き込み失敗の明示**、デプロイ時の **所有権・権限**（[ENVIRONMENT_AND_OPERATIONS.md](ENVIRONMENT_AND_OPERATIONS.md)）、共同作業・運用ドキュメント。以降の更新は `main` および **`develop/v1.2.0`** ブランチの運用に従います。
