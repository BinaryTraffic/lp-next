...
try {
    ...
    if ($jobId !== '') {
        $jobRegistry->heartbeat($jobId, 'generate_lp start');
    ...
    if ($siteMapRaw !== null) {
        ...
        if ($jobId !== '') {
            $jobRegistry->heartbeat($jobId, 'generate_lp generated by site_map');
            lp_job_check_abort($jobRegistry, $jobId, '生成ジョブが停止されました。');
        }
        ...
        if ($jobId !== '') {
            $jobRegistry->heartbeat($jobId, 'generate_lp html ready');
            lp_job_check_abort($jobRegistry, $jobId, '生成ジョブが停止されました。');
        }
