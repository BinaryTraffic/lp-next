...
> **見方:** 上から順に、Ph.0 が「いま届いている土台」、S1 以降がロードマップ。詳しいテーブルは下の「開発スプリント計画」を参照。Web 用ページでは、**先頭付近**に同じ内容を一覧し、**打ち合わせメモ**欄（ブラウザに保存、JSON で受け渡し可）を付けています。
